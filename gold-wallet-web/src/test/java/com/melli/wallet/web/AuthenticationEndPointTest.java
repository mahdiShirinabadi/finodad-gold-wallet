...
public class AuthenticationEndPointTest extends WalletApplicationTests {
    ...
    void login_invalid_credential() throws Exception {
        ...
        login(mockMvc, USERNAME_INCORRECT, PASSWORD_CORRECT, HttpStatus.UNAUTHORIZED,
                StatusRepositoryService.INVALID_USERNAME_PASSWORD, false);
    }
    ...
    void login_ip_not_valid() throws Exception {
        ...
        channelRepositoryService.clearCacheAll();
        login(mockMvc, USERNAME_CORRECT, PASSWORD_CORRECT, HttpStatus.FORBIDDEN, StatusRepositoryService.INVALID_IP_ADDRESS, false);
    ...
    void refresh_success() throws Exception {
        ...
        ACCESS_TOKEN = response.getData().getAccessTokenObject().getToken();
        REFRESH_TOKEN = response.getData().getRefreshTokenObject().getToken();
        BaseResponse<LoginResponse> loginResponse = refresh(mockMvc, REFRESH_TOKEN, USERNAME_CORRECT, HttpStatus.OK, StatusRepositoryService.SUCCESSFUL, true);
    ...
    void refresh_fail_not_found() throws Exception {
        log.info("start login for username ({})", USERNAME_CORRECT);
        refresh(mockMvc, "123i29312190381290312039823", USERNAME_CORRECT, HttpStatus.UNAUTHORIZED, StatusRepositoryService.REFRESH_TOKEN_NOT_FOUND, false);
    }
