...
/**
 * Class Name: StatPhysicalCashOutRepositoryServiceImplementation
 * Author: AI Assistant
 * Date: 1/26/2025
 * Description: Implementation of StatPhysicalCashOutRepositoryService with filtering capabilities
 */
@Service
@Log4j2
@RequiredArgsConstructor
public class StatPhysicalCashOutRepositoryServiceImplementation implements StatPhysicalCashOutRepositoryService {

    private final StatPhysicalCashOutRepository statPhysicalCashOutRepository;

    @Override
    public Page<StatPhysicalCashOutEntity> findAllWithSpecification(Map<String, String> searchCriteria, Pageable pageable) {
        log.info("start findAllWithSpecification with searchCriteria: {}, pageable: {}", searchCriteria, pageable);
        Specification<StatPhysicalCashOutEntity> specification = getPredicate(searchCriteria);
        return statPhysicalCashOutRepository.findAll(specification, pageable);
    }

    public Specification<StatPhysicalCashOutEntity> getPredicate(Map<String, String> searchCriteria) {
    ...
    private List<Predicate> buildPredicatesFromCriteria(Map<String, String> searchCriteria, Root<StatPhysicalCashOutEntity> root, CriteriaBuilder criteriaBuilder) {
    ...
    private void setOrder(CriteriaQuery<?> query, String orderBy, String sortDirection, CriteriaBuilder criteriaBuilder, Root<StatPhysicalCashOutEntity> root) {
