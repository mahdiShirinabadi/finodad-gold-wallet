...
/**
 * Class Name: StatSellRepositoryServiceImplementation
 * Author: AI Assistant
 * Date: 1/26/2025
 * Description: Implementation of StatSellRepositoryService with filtering capabilities
 */
@Service
@Log4j2
@RequiredArgsConstructor
public class StatSellRepositoryServiceImplementation implements StatSellRepositoryService {

    private final StatSellRepository statSellRepository;

    @Override
    public Page<StatSellEntity> findAllWithSpecification(Map<String, String> searchCriteria, Pageable pageable) {
        log.info("start findAllWithSpecification with searchCriteria: {}, pageable: {}", searchCriteria, pageable);
        Specification<StatSellEntity> specification = getPredicate(searchCriteria);
        return statSellRepository.findAll(specification, pageable);
    }

    public Specification<StatSellEntity> getPredicate(Map<String, String> searchCriteria) {
    ...
    private List<Predicate> buildPredicatesFromCriteria(Map<String, String> searchCriteria, Root<StatSellEntity> root, CriteriaBuilder criteriaBuilder) {
        ...
        if (CustomStringUtils.hasText(searchCriteria.get("merchantId"))) {
            predicates.add(criteriaBuilder.equal(root.get("merchantId"), searchCriteria.get("merchantId")));
        }
    ...
    private void setOrder(CriteriaQuery<?> query, String orderBy, String sortDirection, CriteriaBuilder criteriaBuilder, Root<StatSellEntity> root) {
