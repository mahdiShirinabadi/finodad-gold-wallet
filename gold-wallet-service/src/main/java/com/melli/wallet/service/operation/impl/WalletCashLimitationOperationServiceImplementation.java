...
public class WalletCashLimitationOperationServiceImplementation implements WalletCashLimitationOperationService {
    ...
    public void checkCashInLimitation(ChannelEntity channel, WalletEntity wallet, BigDecimal amount, WalletAccountEntity walletAccount) throws InternalServiceException {
        ...
        Boolean enableCashIn = Boolean.parseBoolean(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.ENABLE_CASH_IN, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        ...
        if (Boolean.FALSE.equals(enableCashIn)) {
            ...
            throw new InternalServiceException(st.toString(), StatusRepositoryService.ACCOUNT_DONT_PERMISSION_FOR_CASH_IN, HttpStatus.OK);
        }

        BigDecimal minAmount = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MIN_AMOUNT_CASH_IN, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxAmount = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_AMOUNT_CASH_IN, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxBalance = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_WALLET_BALANCE, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxCashInDaily = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_WALLET_AMOUNT_DAILY_CASH_IN, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        ...
        redisLockService.runAfterLock(key, this.getClass(), () -> {
            ...
            if (amount.compareTo(maxAmount)> 0) {
                ...
                throw new InternalServiceException("CashIn's amount is bigger than minCashIn", StatusRepositoryService.AMOUNT_BIGGER_THAN_MAX, HttpStatus.OK,
                        Map.ofEntries(
            ...
            if (amount.compareTo(minAmount) < 0) {
                ...
                throw new InternalServiceException("CashIn's amount is less than minCashIn", StatusRepositoryService.AMOUNT_LESS_THAN_MIN, HttpStatus.OK, Map.ofEntries(
                        entry("1", Utility.addComma(amount.longValue())),
            ...
            if (walletCashInLimitationRedis == null) {
                ...
                AggregationCashInDTO aggregationPurchaseDTO = requestRepositoryService.findSumAmountCashInBetweenDate(new long[]{walletAccount.getId()}, new Date(), new Date());
            ...
            if ((walletCashInLimitationRedis.getCashInDailyAmount().add(amount).compareTo(BigDecimal.valueOf(maxCashInDaily.longValue()))) > 0) {
                ...
                throw new InternalServiceException("wallet exceeded cashIn the limitation !!!", StatusRepositoryService.WALLET_EXCEEDED_AMOUNT_LIMITATION, HttpStatus.OK, Map.ofEntries(
    ...
    public void checkCashOutLimitation(ChannelEntity channel, WalletEntity wallet, BigDecimal amount, WalletAccountEntity walletAccount) throws InternalServiceException {
        ...
        Boolean enableCashOut = Boolean.parseBoolean(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.ENABLE_CASH_OUT, walletLevelEntity, walletAccountTypeEntity,
        ...
        if (Boolean.FALSE.equals(enableCashOut)) {
            ...
            throw new InternalServiceException("account (" + walletAccount.getAccountNumber() + ") dont permission to cashIn", StatusRepositoryService.ACCOUNT_DONT_PERMISSION_FOR_CASH_OUT, HttpStatus.OK);
        }

        BigDecimal minAmount = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MIN_AMOUNT_CASH_OUT, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxAmount = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_AMOUNT_CASH_OUT, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxCashInDaily = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_WALLET_AMOUNT_DAILY_CASH_OUT, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        ...
        redisLockService.runAfterLock(key, this.getClass(), () -> {
            ...
            if (amount.compareTo(minAmount) < 0) {
                ...
                throw new InternalServiceException("CashOut's amount is less than minCashIn", StatusRepositoryService.AMOUNT_LESS_THAN_MIN, HttpStatus.OK, Map.ofEntries(
                        entry("1", Utility.addComma(amount.longValue())),
                        entry("2", Utility.addComma(minAmount.longValue()))
                ));
            }
            ...
            if (walletCashOutLimitationRedis == null) {
                ...
                AggregationCashOutDTO sumAmountCashOutBetweenDate = requestRepositoryService.findSumAmountCashOutBetweenDate(new long[]{walletAccount.getId()}, new Date(), new Date());
            ...
            if ((walletCashOutLimitationRedis.getCashOutDailyAmount().add(amount).compareTo(BigDecimal.valueOf(maxCashInDaily.longValue()))) > 0) {
                ...
                throw new InternalServiceException("wallet exceeded the limitation !!!", StatusRepositoryService.CASHOUT_EXCEEDED_AMOUNT_DAILY_LIMITATION, HttpStatus.OK, Map.ofEntries(
    ...
    public void checkPhysicalCashOutLimitation(ChannelEntity channel, WalletEntity wallet, BigDecimal quantity, WalletAccountEntity walletAccount) throws InternalServiceException {
        ...
        Boolean enableCashOut = Boolean.parseBoolean(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.ENABLE_PHYSICAL_CASH_OUT, walletLevelEntity, walletAccountTypeEntity,
                walletAccountCurrencyEntity, walletTypeEntity));
        ...
        if (Boolean.FALSE.equals(enableCashOut)) {
            log.error("checkPhysicalCashOutLimitation: physical cash out for channel {}, walletAccountCurrencyEntity ({}) and walletAccountTypeEntity ({}) walletTypeEntity ({}) is not permission !!!", channel,
                    walletAccount.getWalletAccountCurrencyEntity().getName(),  walletAccount.getWalletAccountTypeEntity().getName(), walletAccount.getWalletEntity().getWalletTypeEntity().getName());
            throw new InternalServiceException("account (" + walletAccount.getAccountNumber() + ") dont permission to cashIn", StatusRepositoryService.ACCOUNT_DONT_PERMISSION_FOR_PHYSICAL_CASH_OUT, HttpStatus.OK);
        }

        BigDecimal minQuantity = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MIN_QUANTITY_PHYSICAL_CASH_OUT, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxQuantity = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_QUANTITY_PHYSICAL_CASH_OUT, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        BigDecimal maxPhysicalCashOutDaily = new BigDecimal(limitationGeneralCustomRepositoryService.getSetting(channel, LimitationGeneralService.MAX_WALLET_QUANTITY_DAILY_PHYSICAL_CASH_OUT, walletLevelEntity, walletAccountTypeEntity, walletAccountCurrencyEntity, walletTypeEntity));
        ...
        redisLockService.runAfterLock(key, this.getClass(), () -> {
            ...
            if (quantity.compareTo(minQuantity) < 0) {
                log.error("checkPhysicalCashOutLimitation: PhysicalCashOut's quantity({}) for wallet({}), is less than minPhysicalCashOut({}) !!!", quantity, wallet.getNationalCode(), minQuantity);
                throw new InternalServiceException("CashOut's amount is less than minCashOut", StatusRepositoryService.QUANTITY_LESS_THAN_MIN, HttpStatus.OK, Map.ofEntries(
            ...
            if (quantity.compareTo(maxQuantity)> 0) {
                log.error("checkPhysicalCashOutLimitation: CashOut's amount({}) for wallet({}), is bigger than maxCashOut({}) !!!", quantity, wallet.getNationalCode(), maxQuantity);
                throw new InternalServiceException("CashOut's amount is bigger than maxCashOut", StatusRepositoryService.QUANTITY_BIGGER_THAN_MAX, HttpStatus.OK,
                        Map.ofEntries(
                                entry("1", Utility.addComma(quantity.longValue())),
            ...
            if (walletPhysicalCashOutLimitationRedis == null) {
                ...
                AggregationCashOutDTO sumAmountCashOutBetweenDate = requestRepositoryService.findSumAmountPhysicalCashOutBetweenDate(new long[]{walletAccount.getId()}, new Date(), new Date());
                ...
                walletPhysicalCashOutLimitationRedis.setCashOutDailyQuantity(new BigDecimal(sumAmountCashOutBetweenDate.getSumPrice()));
            ...
            log.info("SumPhysicalCashOutAmount for wallet({}) in date: ({}) is: {}", wallet.getNationalCode(), currentDate, walletPhysicalCashOutLimitationRedis.getCashOutDailyQuantity());

            if ((walletPhysicalCashOutLimitationRedis.getCashOutDailyQuantity().add(quantity).compareTo(BigDecimal.valueOf(maxPhysicalCashOutDaily.longValue()))) > 0) {
                log.error("wallet({}) on channel ({}) , exceeded amount limitation in cashOut!!! SumPhysicalCashOutAmount is: {}", wallet.getNationalCode(), wallet.getOwner().getId(), walletPhysicalCashOutLimitationRedis.getCashOutDailyQuantity());
                throw new InternalServiceException("wallet exceeded the limitation !!!", StatusRepositoryService.PHYSICAL_CASHOUT_EXCEEDED_AMOUNT_DAILY_LIMITATION, HttpStatus.OK, Map.ofEntries(
                        entry("1", Utility.addComma(walletPhysicalCashOutLimitationRedis.getCashOutDailyQuantity().add(quantity).longValue())),
                        entry("2", Utility.addComma((maxPhysicalCashOutDaily.longValue())))
                ));
            }
            return null;
        }, null);
    ...
    public void updatePhysicalCashOutLimitation(WalletAccountEntity walletAccountEntity, BigDecimal quantity) throws InternalServiceException {
        ...
        redisLockService.runAfterLock(key, this.getClass(), () -> {
            ...
            if (walletPhysicalCashOutLimitationRedis == null) {
                ...
                walletPhysicalCashOutLimitationRedis.setCashOutDailyQuantity(quantity);
            } else {
                walletPhysicalCashOutLimitationRedis.setCashOutDailyCount(walletPhysicalCashOutLimitationRedis.getCashOutDailyCount() + 1);
                walletPhysicalCashOutLimitationRedis.setCashOutDailyQuantity(walletPhysicalCashOutLimitationRedis.getCashOutDailyQuantity().add(quantity));
            }
