...
/**
 * Class Name: StatBuyRepositoryServiceImplementation
 * Author: AI Assistant
 * Date: 1/26/2025
 * Description: Implementation of StatBuyRepositoryService with filtering capabilities
 */
@Service
@Log4j2
@RequiredArgsConstructor
public class StatBuyRepositoryServiceImplementation implements StatBuyRepositoryService {

    private final StatBuyRepository statBuyRepository;

    @Override
    public Page<StatBuyEntity> findAllWithSpecification(Map<String, String> searchCriteria, Pageable pageable) {
        log.info("start findAllWithSpecification with searchCriteria: {}, pageable: {}", searchCriteria, pageable);
        Specification<StatBuyEntity> specification = getPredicate(searchCriteria);
        return statBuyRepository.findAll(specification, pageable);
    }

    public Specification<StatBuyEntity> getPredicate(Map<String, String> searchCriteria) {
    ...
    private List<Predicate> buildPredicatesFromCriteria(Map<String, String> searchCriteria, Root<StatBuyEntity> root, CriteriaBuilder criteriaBuilder) {
    ...
    private void setOrder(CriteriaQuery<?> query, String orderBy, String sortDirection, CriteriaBuilder criteriaBuilder, Root<StatBuyEntity> root) {
