...
public class WalletAccountTypeRepositoryServiceImplementation implements WalletAccountTypeRepositoryService {
    ...
    public WalletAccountTypeEntity getById(Long id) throws InternalServiceException {
        ...
        if (reportEntity == null) {
            ...
            throw new InternalServiceException("Wallet account type not found", StatusRepositoryService.WALLET_ACCOUNT_TYPE_NOT_FOUND, HttpStatus.OK);
        }
        return walletAccountTypeMapper.toWalletAccountTypeEntity(reportEntity);
    }
    ...
    public WalletAccountTypeEntity getByIdManaged(Long id) throws InternalServiceException {
        return walletAccountTypeRepository.findById(id).orElseThrow(() -> {
            log.error("wallet account type with id ({}) not found", id);
            return new InternalServiceException("Wallet account type not found", StatusRepositoryService.WALLET_ACCOUNT_TYPE_NOT_FOUND, HttpStatus.OK);
        });
    }
}
